...
TEST(SmfPolicyManagerTest, ParsePolicyDecision_HandlesInvalidAndValidJson) {
  ...
  // TS 29.512: the decision always arrives wrapped in an SmPolicyNotification,
  // so a bare decision is not accepted.
  nlohmann::json unwrapped = R"({
    "pccRules": {}
  })"_json;
  EXPECT_FALSE(smf_policy_manager::parse_policy_decision(unwrapped, decision));
}
