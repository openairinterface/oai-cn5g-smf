...
pfcp::create_far make_paging_create_far(const pfcp::far_id_t& far_id) {
  ...
  // Exactly one of DROP/FORW/BUFF may be set, and NOCP only together with
  // BUFF, so there are no Forwarding Parameters here.
  apply_action.buff = 1;
  apply_action.nocp = 1;
...
pfcp::create_pdr make_paging_create_pdr(
  ...
  // downlink: the UE IPv4 address is the packet's destination (S/D = 1)
  ue_ip_address.v4           = 1;
  ue_ip_address.sd           = 1;
  ...
  // has to be the FAR created in the same message
