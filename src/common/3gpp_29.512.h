...
// Table 5.7.3-1: Application errors when PCF acts as a server @3GPP TS 29.512
// V17.6.0
enum class pcf_server_application_error_e {
  UNKNOWN_ERROR = 0,
  USER_UNKNOWN =
      1,  // The end user specified in the request is unknown to the PCF
  ERROR_INITIAL_PARAMETERS = 2,  // The set of session/subscriber info needed by
                                 // the PCF is incomplete or erroneous
  ERROR_TRIGGER_EVENT = 3,  // The session info trigger update is incoherent
                            // with previous session info
  ERROR_TRAFFIC_MAPPING_INFO_REJECTED =
      4,  // The PCF does not accept one or more traffic mapping filters from
          // the consumer
  ERROR_CONFLICTING_REQUEST = 5,  // UE-initiated request covers existing
                                  // network-initiated allocation filters
  LATE_OVERLAPPING_REQUEST = 6,  // Collides with an existing Policy Association
                                 // with a more recent timestamp
  POLICY_CONTEXT_DENIED = 7,     // Request rejected due to operator policies
                                 // and/or local configuration
  VALIDATION_CONDITION_NOT_MET =
      8,                    // BDT policy validation conditions are not met
  PENDING_TRANSACTION = 9,  // Request received while another transaction is
                            // ongoing on the same policy association
  INVALID_BDT_POLICY = 10,  // Background data transfer policy is invalid
  EXCEEDED_UE_SLICE_DATA_RATE = 11,  // Authorized data rate exceeds consumed
                                     // data rate for that UE and network slice
  EXCEEDED_SLICE_DATA_RATE =
      12,  // Authorized data rate exceeds consumed data rate for that slice
  POLICY_ASSOCIATION_NOT_FOUND = 13  // No policy association corresponding to
                                     // the request exists in the PCF
};
...
// Table 5.7.3-2: Application errors when NF service consumer (SMF) acts as a
// server to receive a notification @3GPP TS 29.512 V17.6.0
enum smf_server_application_error_e {
  UNKNOWN_ERROR = 0,
  PCC_RULE_EVENT =
      1,  // All dynamic PCC rules in the request cannot be installed/activated
  PCC_QOS_FLOW_EVENT = 2,  // All dynamic PCC rules in the request cannot be
                           // enforced or modified successfully
  UE_STATUS_SUSPEND =
      3,  // UE status is suspended; policy decisions cannot be enforced
  RULE_PERMANENT_ERROR =
      4,  // All dynamic PCC and/or session rules cannot be installed/activated
  RULE_TEMPORARY_ERROR = 5,  // All dynamic PCC and/or session rules cannot be
                             // enforced or modified temporarily
  PENDING_TRANSACTION = 6,   // Request received on association while SMF has an
                             // ongoing transaction
  AN_GW_FAILED = 7           // Policy decisions cannot be enforced because the
                             // AN-Gateway has failed
};

static const std::vector<std::string> smf_server_application_error_e2str = {
    "UNKNOWN_ERROR",       "PCC_RULE_EVENT",       "PCC_QOS_FLOW_EVENT",
    "UE_STATUS_SUSPEND",   "RULE_PERMANENT_ERROR", "RULE_TEMPORARY_ERROR",
    "PENDING_TRANSACTION", "AN_GW_FAILED"};
...
enum class policy_control_request_trigger_e {
  ...
  DDN_DELIVERY_STATUS_CANCELLATION =
      23  // Cancel subscription to DDN delivery status
};
...
static const std::vector<std::string> session_rule_failure_code_e2str = {
    "UNKNOWN_SESS_FAILURE",   "UNK_SESS_RULE_ID",     "INCORRECT_UM",
    "SESS_AMBR_OUT_OF_RANGE", "DEF_QOS_OUT_OF_RANGE", "AN_GW_FAILED"};
...
static const std::vector<std::string> steer_mode_value_e2str = {
    "UNKNOWN_STEER_MODE", "ACTIVE_STANDBY", "LOAD_BALANCING", "SMALLEST_DELAY",
    "PRIORITY_BASED"};
